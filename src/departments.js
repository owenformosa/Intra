document.addEventListener('DOMContentLoaded', () => {
  ...
  fetch('./departments.json')   // 🔧 fixed relative path
    .then(res => res.json())
    .then(data => {
      console.log("Departments JSON:", data);
      const departments = data.departments || data; // supports both {departments:[]} or []
  ...
  function createCard(dep, index) {
    ...
    return `
      <div class="card w-80 bg-gradient-to-br ${gradient.from} ${gradient.to} shadow-lg p-6 rounded-xl
        ...
        <a href="./team.html?id=${encodeURIComponent(dep.name)}"   <!-- 🔧 fixed -->
